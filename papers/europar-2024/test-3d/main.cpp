...
int main(int argc, char **argv) {
  ...
  int *h_a = static_cast<int *>(
      omp_target_alloc(N * N * sizeof(int), omp_get_initial_device()));
...
#pragma omp target teams ompx_bare num_teams(N, 1, 1)       \
    ...
    ompx_dyn_cgroup_mem(vector_size *team_size * sizeof(int))
  {
    ...
    double *buf = static_cast<double *>(llvm_omp_target_dynamic_shared_alloc());
    ...
    for (int j = threadIdy; j < N; j += blockDimy) {
      ...
      if (threadIdx == 0) {
        for (int tid = 0; tid < blockDimx; ++tid) update += buf[tid];
