...
class SNA {
 ...
 public:
  ...
  SNAcomplex_View4DL dalist_gpu = SNAcomplex_View4DL("dalist_gpu", 0, 0, 0, 0);
  SNAcomplex_View4DL dblist_gpu = SNAcomplex_View4DL("dblist_gpu", 0, 0, 0, 0);
  ...
  void operator()(const ComputeYiTag, const int iter) const {
    ...
    for (int ib = 0; ib < nb; ib++) {
      ...
      for (int ia = 0; ia < na; ia++) {
        ...
        suma1_r +=
            cgblock[icga] *
            (ulisttot(jju1 + ma1, natom).re * ulisttot(jju2 + ma2, natom).re -
             ulisttot(jju1 + ma1, natom).im * ulisttot(jju2 + ma2, natom).im);
        suma1_i +=
            cgblock[icga] *
            (ulisttot(jju1 + ma1, natom).re * ulisttot(jju2 + ma2, natom).im +
             ulisttot(jju1 + ma1, natom).im * ulisttot(jju2 + ma2, natom).re);
#else
        suma1_r +=
            cgblock[icga] *
  ...
  void operator()(const ComputeDUiTag, const int iter) const {
    ...
    for (int j = 1; j <= twojmax; j++) {
      ...
      for (int mb = 0; 2 * mb < j; mb++) {
        for (int k = 0; k < 3; ++k) dulist(natom, nbor, jjdu, k) = {0.0, 0.0};
  ...
  KOKKOS_INLINE_FUNCTION
  void operator()(const ComputeUiTagGPU, const member_type team_member) const {
    int iter = team_member.league_rank() * team_member.team_size() +
               team_member.team_rank();
          ...
          for (int j = 1; j <= jbend; j++) {
            ...
            for (ma = 0; ma < j; ma++) {
              ...
              const SNAcomplex ulist_prev =
                  ulist_scratch[tid_index + ma * vector_length_loc + natom_mod];

              // ulist_tmp += rootpq * a.conj * ulist_prev
              SNADOUBLE rootpq = rootpqarray(j - ma, j);  // mb = 0
              ulist_tmp.re +=
                  rootpq * (a.re * ulist_prev.re + a.im * ulist_prev.im);
              ulist_tmp.im +=
                  rootpq * (a.re * ulist_prev.im - a.im * ulist_prev.re);
              ...
              ulist_tmp.re =
                  -rootpq * (b.re * ulist_prev.re + b.im * ulist_prev.im);
              ulist_tmp.im =
                  -rootpq * (b.re * ulist_prev.im - b.im * ulist_prev.re);
            }
  ...
  KOKKOS_INLINE_FUNCTION
  void operator()(const TransformUiTagGPU, const int natom_mod, const int idxu,
                  const int natom_div) const {
    const int natom = natom_mod + natom_div * vector_length;
    if (natom >= num_atoms) return;
    ...
    SNAcomplex utot = {ulisttot_re_gpu(natom_mod, map.idxu_half, natom_div),
...
#pragma unroll
      for (int ia = 0; ia < na; ia++) {
        ...
        suma1_r +=
            cgblock[icga] * (utot_1.re * utot_2.re - utot_1.im * utot_2.im);
        suma1_i +=
            cgblock[icga] * (utot_1.re * utot_2.im + utot_1.im * utot_2.re);
        ma1++;
        ma2--;
        icga += j2;
      }  // end loop over ia
  ...
  template <int dir>
  KOKKOS_INLINE_FUNCTION void operator()(const ComputeFusedDeiDrjTagGPU<dir>,
    ...
    ScratchViewType ulist_scratch(team_member.team_scratch(0),
                                  team_member.team_size() * PerTeamScratch);
    ScratchViewType dulist_scratch(team_member.team_scratch(0),
                                   team_member.team_size() * PerTeamScratch);
    ...
    Kokkos::parallel_for(
        ...
        [&](const int natom_mod) {
          ...
          const SNAcomplex a   = alist_gpu(natom_mod, nbor, natom_div);
          const SNAcomplex b   = blist_gpu(natom_mod, nbor, natom_div);
          const SNAcomplex da  = dalist_gpu(natom_mod, nbor, natom_div, dir);
          const SNAcomplex db  = dblist_gpu(natom_mod, nbor, natom_div, dir);
          const SNADOUBLE sfac = sfaclist_gpu(natom_mod, nbor, natom_div, 0);
          const SNADOUBLE dsfacu =
              sfaclist_gpu(natom_mod, nbor, natom_div, dir + 1);  // dsfac * u
          ...
          for (int j = 1; j <= jbend; j++) {
            ...
            for (ma = 0; ma < j; ma++) {
              ...
              dulist_tmp.re +=
                  rootpq * (da.re * ulist_prev.re + da.im * ulist_prev.im +
                            a.re * dulist_prev.re + a.im * dulist_prev.im);
              dulist_tmp.im +=
                  rootpq * (da.re * ulist_prev.im - da.im * ulist_prev.re +
                            a.re * dulist_prev.im - a.im * dulist_prev.re);

              // store ulist, dulist tmp for the next level
              ulist_scratch[tid_index + ma * vector_length_loc + natom_mod] =
                  ulist_tmp;
              dulist_scratch[tid_index + ma * vector_length_loc + natom_mod] =
                  dulist_tmp;
              ...
              // product rule of above
              dulist_tmp.re =
                  -rootpq * (db.re * ulist_prev.re + db.im * ulist_prev.im +
                             b.re * dulist_prev.re + b.im * dulist_prev.im);
              dulist_tmp.im =
                  -rootpq * (db.re * ulist_prev.im - db.im * ulist_prev.re +
          ...
          for (j = jbend + 1; j <= j_half_way; j++) {
            ...
            for (ma = 0; ma < j; ma++) {
              ...
              const SNAcomplex ylist_value =
                  SNAcomplex(ylist_re_gpu(natom_mod, jjup + ma, natom_div),
          ...
          for (int ma = 0; ma < j; ma++) {
            ...
            const SNAcomplex du_product = {
                ...
                dsfacu * ulist_prev.im + sfac * dulist_prev.im};
            dedr_sum +=
                du_product.re * ylist_value.re + du_product.im * ylist_value.im;
          }

          Kokkos::atomic_add(
              &(dedr(natom_mod + vector_length_loc * natom_div, nbor, dir)),
              2. * dedr_sum);
        });
  }
  ...
  void addself_uarraytot() {
    ...
    Kokkos::parallel_for("AddSelfUarraytot", policy_AddSelfUarraytot, *this);
  }
  ...
  void compute_ui() {
    ...
    auto policy_Ui = Kokkos::TeamPolicy<ComputeUiTag>(numBlocks, numThreads);
  ...
  void compute_yi() {
    ...
    auto policy_Yi = Kokkos::RangePolicy<ComputeYiTag>(0, num_atoms * idxz_max);
  ...
  void compute_fused_deidrj_gpu() {
    ...
    int scratch_size = ScratchViewType::shmem_size(team_size * PerTeamScratch);

    {
      ...
      Kokkos::parallel_for("ComputeFusedDeiDrjGPU<0>", policy_fused_deidrj,
                           *this);
    }

    {
      ...
      Kokkos::parallel_for("ComputeFusedDeiDrjGPU<1>", policy_fused_deidrj,
                           *this);
    }

    {
      ...
      Kokkos::parallel_for("ComputeFusedDeiDrjGPU<2>", policy_fused_deidrj,
                           *this);
    }
